  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 4, marginBottom: 16, background: t.surface, borderRadius: 10, padding: 3, flexWrap: "wrap" }}>
        ...
        <button onClick={onLogTime} style={{ display: "flex", alignItems: "center", gap: 4, padding: "7px 10px", borderRadius: 8, border: "none", background: "transparent", color: t.moss, fontSize: 12, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}><Plus size={13} /> Log</button>
      </div>
      ...
      {tab === "chart" && (
        <div>
          <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
            {RANGES.map((r) => (
              <button key={r.key} onClick={() => setRange(r.key)} style={{ flex: 1, padding: "6px 4px", borderRadius: 999, border: `1px solid ${range === r.key ? t.moss : t.line}`, background: range === r.key ? t.moss : "transparent", color: range === r.key ? t.bg : t.sub, fontSize: 11, fontWeight: 600, cursor: "pointer" }}>{r.label}</button>
            ))}
          </div>

          <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            <StatCard t={t} label="Active days" value={`${rangeStats.activeDays} / ${rangeStats.totalDays}`} accent={t.moss} />
            <StatCard t={t} label="Days lost to gaps" value={rangeStats.gapDays} accent={t.clay} />
            <StatCard t={t} label={`Hit ${(dailyGoalMinutes / 60).toFixed(dailyGoalMinutes % 60 ? 1 : 0)}h target`} value={`${rangeStats.goalHitDays} / ${rangeStats.totalDays}`} accent={t.moss} />
          </div>
          <div style={{ fontSize: 10.5, color: t.sub, marginBottom: 14 }}>Change your daily target in the <strong>Goal</strong> tab above, or in Settings.</div>

          <div className="sf-scroll" style={{ height: 180, marginBottom: 16, background: t.surface, borderRadius: 14, padding: "12px 4px 0", overflowX: "auto" }}>
            <div style={{ width: chartPxWidth, height: "100%" }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={rangeData} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  <XAxis dataKey="label" tick={{ fontSize: 9.5, fill: t.sub }} axisLine={false} tickLine={false} interval={tickInterval} />
                  <YAxis tick={{ fontSize: 10, fill: t.sub }} axisLine={false} tickLine={false} width={30} />
                  <Tooltip cursor={{ fill: t.line, opacity: 0.4 }} contentStyle={{ background: t.surface, border: `1px solid ${t.line}`, borderRadius: 8, fontSize: 12 }} formatter={(v) => [`${v} min`, "Focused"]} />
                  <Bar dataKey="minutes" fill="url(#barGradMoss)" radius={[3, 3, 0, 0]} maxBarSize={28} />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          {range !== "7d" && <div style={{ fontSize: 10.5, color: t.sub, marginTop: -10, marginBottom: 14 }}>Scroll to see every day in this range.</div>}
          <div style={{ display: "flex", justifyContent: "space-between", padding: "14px 4px", borderTop: `1px solid ${t.line}` }}>
            <span style={{ fontSize: 13, color: t.sub }}>All-time focused</span>
            <span style={{ fontSize: 13, fontWeight: 600 }}>{Math.floor(totalAll / 60)}h {totalAll % 60}m</span>
          </div>
        </div>
      )}
      {tab === "records" && <RecordsView t={t} sessions={sessions} sleepSettings={sleepSettings} />}
      {tab === "goal" && <GoalCalendar t={t} sessions={sessions} dailyGoalMinutes={dailyGoalMinutes} setDailyGoalMinutes={setDailyGoalMinutes} />}
      {tab === "compare" && <CompareView t={t} sessions={sessions} compareDates={compareDates} setCompareDates={setCompareDates} />}
    </div>
  );
}
